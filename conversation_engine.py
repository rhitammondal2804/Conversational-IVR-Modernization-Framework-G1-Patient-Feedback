...
AFFIRM_KEYWORDS = {"yes", "yeah", "yep", "correct", "right", "sure", "absolutely",
                   "of course", "definitely", "confirm", "true", "yup", "ok", "okay"}
...
SKIP_KEYWORDS   = {"skip", "none", "nothing", "no comment", "no comments", "pass", "next"}
...
class AIConversationEngine:
    ...
    def _extract_intent(self, text: str, state: IVRState) -> Optional[str]:
        ...
        # Submit / confirm (before start, to catch "confirm" before "continue")
        if any(kw in words for kw in SUBMIT_KEYWORDS):
            return "submit"
        # Review / go back — word-level check first, then phrase for "go back"
        if "go back" in text or any(kw in words for kw in REVIEW_KEYWORDS):
            return "review"
        # Start feedback — word-level
        ...
        if any(kw in words for kw in SKIP_KEYWORDS):
            return "skip"
        # Affirm / deny — whole-word to avoid "no" inside "know", etc.
